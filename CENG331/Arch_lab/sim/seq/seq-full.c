...
long long gen_need_regids()
{
    return ((icode) == (I_RRMOVQ) || (icode) == (I_ALU) || (icode) == 
      (I_PUSHQ) || (icode) == (I_POPQ) || (icode) == (I_IRMOVQ) || (icode)
       == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == (I_LEAQ));
}
...
long long gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVQ) || (icode) == 
        (I_MRMOVQ) || (icode) == (I_LEAQ)) ? (rb) : ((icode) == (I_PUSHQ)
...
long long gen_dstE()
{
    return (((icode) == (I_LEAQ)) ? (ra) : (((icode) == (I_RRMOVQ)) & 
        (cond)) ? (rb) : ((icode) == (I_IRMOVQ) || (icode) == (I_ALU)) ? 
      (rb) : ((icode) == (I_PUSHQ) || (icode) == (I_POPQ) || (icode) == 
        (I_CALL) || (icode) == (I_RET)) ? (REG_RSP) : (REG_NONE));
}
...
long long gen_aluA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_ALU)) ? (vala) : (
        ...
        (I_MRMOVQ) || (icode) == (I_LEAQ)) ? (valc) : ((icode) == (I_CALL)
...
long long gen_aluB()
{
    return (((icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == 
        (I_ALU) || (icode) == (I_CALL) || (icode) == (I_PUSHQ) || (icode)
         == (I_RET) || (icode) == (I_POPQ) || (icode) == (I_LEAQ)) ? (valb)
