...
## You can implement the leaq instruction if you like. Pretty easy since
## most of the issues are already handled for you. Could be useful in your
## absrev.ys code!
...
# Stage             LEAq C(rB),rA
####################################################################
# Fetch[checked]    icode:fun   <- M1[PC]
#                   rA:rB       <- M1[PC+1]
#                   valC        <- M8[PC+2]
#                   valP        <- PC+10
####################################################################
# Decode            DO NOT SET valB not needed
#                   valB        <- R[rB]                srcB
#                   dstE        <- rA
####################################################################
# Execute           valE        <- valC + valB         aluA,aluB
#                       DO NOT SET CC 
####################################################################
# Memory                DO NOT MAKE MEMORY OPS
####################################################################
# Write Back        R[rA]       <- valE                 dstE(left(rA))
#                       DO NOT                          dstM
####################################################################
# PC Update         PC          <- valP                 shoud be automatic
####################################################################
...
quote '#include <stdio.h>'
    ...
    # Instruction code for leaq instruction
    wordsig ILEAQ   'I_LEAQ'
    # Instruction code for cmpq instruction
    wordsig IICMPQ   'I_ICMPQ'
    ...
    word d_srcA = [
        D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, IICMPQ  } : D_rA;
    ...
    ## Should the condition codes be updated?
        bool set_cc = E_icode in { IOPQ, IICMPQ } &&
